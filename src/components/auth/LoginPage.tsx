...
interface LoginPageProps {
    onLogin: (emailOrUsername: string, password: string) => boolean;
}
...
export const LoginPage = ({ onLogin }: LoginPageProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (loginType === 'email' && emailOrUsername.includes('@creosolutions.tech')) {
                const bool = onLogin(emailOrUsername, password);

                if (!bool) setError('Invalid email or password');
            } else {
                const bool = onLogin(emailOrUsername, password);

                if (!bool) setError('Invalid username or password');
            }
        } catch (err) {
